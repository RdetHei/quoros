    ...
    <form action="{{ route('writer.novels.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-3xl p-8 shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800">
            <div class="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100 dark:border-slate-800">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400">Upload cover yang menarik untuk novel kamu</p>
                </div>
            </div>
            ...
            <div class="flex flex-col lg:flex-row gap-8 items-start">
                ...
                <div class="flex-shrink-0">
                    <div id="cover-preview-container" class="w-40 h-[240px] rounded-2xl overflow-hidden bg-slate-100 dark:bg-slate-800 border-2 border-dashed border-slate-200 dark:border-slate-700 shadow-lg relative group">
                        <img id="cover-preview" class="w-full h-full object-cover hidden" onerror="this.onerror=null; this.src='/error.png'">
                        <div id="cover-placeholder" class="w-full h-full flex flex-col items-center justify-center p-4 text-center">
                            ...
                            <p class="text-[10px] font-bold text-slate-400 uppercase">Cover Preview</p>
                ...
                <div class="flex-grow w-full">
                    <label for="cover_image" class="flex flex-col items-center justify-center w-full h-44 border-2 border-slate-200 dark:border-slate-700 border-dashed rounded-3xl cursor-pointer bg-slate-50 dark:bg-slate-800/50 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all">
                        ...
                        <input id="cover_image" name="cover_image" type="file" class="hidden" accept="image/*"
                               onchange="initCropper(this, 'cover-preview', { aspectRatio: 3/4, width: 600, height: 800 }); document.getElementById('cover-placeholder').classList.add('hidden'); document.getElementById('cover-preview').classList.remove('hidden');">
                    </label>
                </div>
            </div>
        </div>
            ...
            <div class="flex items-center justify-between mb-6 pb-4 border-b border-slate-100 dark:border-slate-800">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500 dark:text-slate-400">Tambahkan karakter yang akan muncul di halaman novel (Maks 10)</p>
                    </div>
                </div>
                <button type="button" id="add-character-btn" class="px-4 py-2 bg-slate-900 text-white text-xs font-bold rounded-xl hover:bg-slate-800 transition-all flex items-center gap-2">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-3xl p-8 shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800">
            <div class="flex flex-col sm:flex-row gap-4">
                ...
                <button type="submit" class="flex-grow py-4 bg-emerald-600 text-white font-bold rounded-2xl hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-200 dark:shadow-none order-1 sm:order-2">
                    Publikasikan Novel
                </button>
            </div>
        </div>
                ...
                <div class="lg:col-span-2 space-y-4">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="character_name[]" class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/40 transition-all" placeholder="Contoh: Akiro">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Peran</label>
                            <input type="text" name="character_role[]" class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/40 transition-all" placeholder="Contoh: Protagonis">
                        </div>
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Deskripsi Singkat</label>
                        <textarea name="character_description[]" rows="3" class="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/40 transition-all" placeholder="Deskripsikan karakter ini..."></textarea>
                    </div>
                </div>
            </div>
        `;
