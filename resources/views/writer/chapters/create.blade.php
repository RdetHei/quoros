    ...
    <div class="bg-white dark:bg-slate-900 rounded-3xl p-8 md:p-10 shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800">
        ...
        <div x-show="uploadMode === 'bulk'">
            ...
            <div x-show="step === 'upload'" class="space-y-8" x-transition>
                <div class="p-8 bg-indigo-50 dark:bg-indigo-900/20 rounded-[2rem] border border-indigo-100 dark:border-indigo-800/50">
                    <div class="flex items-start gap-4 mb-6">
                        ...
                        <div class="flex-grow">
                            <h3 class="text-lg font-bold text-slate-900 dark:text-white">Bulk Upload Dokumen</h3>
                            <p class="text-sm text-slate-500 dark:text-slate-400">Sistem akan otomatis membedah file EPUB, DOCX, atau PDF Anda.</p>
                        </div>
                    ...
                    <div class="space-y-4">
                        <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest">Pilih File (EPUB, DOCX, PDF)</label>
                        <div class="flex items-center justify-center w-full">
                            <label class="flex flex-col items-center justify-center w-full h-48 border-2 border-indigo-200 dark:border-indigo-800 border-dashed rounded-3xl cursor-pointer bg-white dark:bg-slate-900 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-all group">
                                <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                    ...
                                    <p class="text-sm text-slate-500 dark:text-slate-400 font-bold" x-text="docFile ? docFile.name : 'Seret atau klik untuk pilih file'"></p>
                ...
                <div class="flex flex-col sm:flex-row gap-4 pt-6">
                    <button @click="startParsing()" :disabled="!docFile || isParsing" class="flex-grow py-4 bg-indigo-600 text-white font-bold rounded-2xl hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-200 dark:shadow-none flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
                        ...
                        <span x-text="isParsing ? 'Menganalisis Dokumen...' : 'Proses & Ekstrak Chapter'"></span>
                    </button>
                    <a href="{{ route('novels.show', $novel->slug) }}" class="px-8 py-4 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-bold rounded-2xl border border-slate-200 dark:border-slate-700 text-center hover:bg-slate-50 dark:hover:bg-slate-700 transition-all">Batal</a>
                </div>
            </div>
            ...
            <div x-show="step === 'processing'" class="space-y-6" x-transition>
                <div class="bg-slate-50 dark:bg-slate-800/50 rounded-[2.5rem] p-8 border border-slate-200 dark:border-slate-700">
                    <div class="flex items-center justify-between mb-8">
                        <div>
                            <h3 class="text-2xl font-black text-slate-900 dark:text-white mb-1">Mengupload Chapter...</h3>
                            <p class="text-sm text-slate-500 dark:text-slate-400">Mohon jangan tutup halaman ini hingga proses selesai.</p>
                        </div>
                    ...
                    <div class="flex items-center gap-4 p-5 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm mb-8">
                        ...
                        <div class="flex-grow">
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Sedang Mengunggah</p>
                            <h4 class="text-base font-bold text-slate-800 dark:text-white line-clamp-1" x-text="chapters[currentIndex] ? chapters[currentIndex].title : 'Menyiapkan...'"></h4>
                        </div>
                        ...
                        <template x-for="(chapter, index) in chapters" :key="index">
                            <div class="flex items-center justify-between p-3 rounded-xl transition-colors" 
                                ...
                                <span class="text-[10px] font-bold uppercase tracking-wider" 
                                      ...
                                      x-text="index < currentIndex ? 'Berhasil' : (index === currentIndex ? 'Proses' : 'Menunggu')"></span>
                            </div>
                        </template>
                    </div>
                </div>
            ...
            <div x-show="step === 'finished'" class="text-center py-12" x-transition>
                ...
                <h2 class="text-3xl font-black text-slate-900 dark:text-white mb-3">Impor Selesai!</h2>
                <p class="text-slate-500 dark:text-slate-400 mb-10 max-w-md mx-auto">Selamat! <span class="font-bold text-slate-800 dark:text-slate-200" x-text="chapters.length"></span> chapter telah berhasil diekstrak dan diunggah ke novel Anda.</p>
