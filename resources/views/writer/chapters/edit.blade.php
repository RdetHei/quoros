...
<div class="max-w-4xl mx-auto my-12">
    <div class="bg-white dark:bg-slate-900 rounded-3xl p-8 md:p-10 shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800">
        <div class="mb-8">
            ...
            <p class="text-slate-500 dark:text-slate-400">Editing chapter <span class="font-bold text-indigo-600">{{ $chapter->title }}</span> on novel <span class="font-bold text-indigo-600">{{ $novel->title }}</span>.</p>
        </div>

        <form action="{{ route('writer.chapters.update', [$novel->id, $chapter->id]) }}" method="POST" enctype="multipart/form-data" class="space-y-8" x-data="{ status: '{{ old('status', $chapter->status) }}' }">
            @csrf
            @method('PUT')
            <div>
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="15" 
                    ...
                    placeholder="Write your story here...">{{ old('content', $chapter->content) }}</textarea>
                @error('content')
                    <p class="mt-2 text-xs text-red-500 font-medium">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                <label for="file" class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Change Document File (Optional)</label>
                <div class="flex flex-col gap-4">
                    @if($chapter->file_path)
                        <div class="flex items-center gap-3 p-4 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 rounded-2xl">
                            ...
                            <div class="flex-grow">
                                <p class="text-sm font-bold text-emerald-800 dark:text-emerald-400">Current File</p>
                                <a href="{{ asset('storage/' . $chapter->file_path) }}" target="_blank" class="text-xs text-emerald-600 hover:underline">View File</a>
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="flex items-center justify-center w-full">
                        <label for="file" class="flex flex-col items-center justify-center w-full h-32 border-2 border-slate-200 dark:border-slate-700 border-dashed rounded-3xl cursor-pointer bg-slate-50 dark:bg-slate-800/50 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all">
                            <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                ...
                                <p class="text-sm text-slate-500 dark:text-slate-400 font-bold text-center">Click to upload new file</p>
        ...
        <div class="mt-12 pt-8 border-t border-rose-100 dark:border-rose-900/30">
            <div class="bg-rose-50 dark:bg-rose-900/10 p-6 rounded-3xl border border-rose-100 dark:border-rose-900/30 flex flex-col md:flex-row items-center justify-between gap-6">
                <div>
                    <h3 class="text-lg font-bold text-rose-800 dark:text-rose-400 mb-1">Danger Zone</h3>
                    <p class="text-sm text-rose-600/70 dark:text-rose-400/60">Deleting a chapter is permanent and cannot be undone.</p>
                </div>
                <form action="{{ route('writer.chapters.destroy', [$novel->id, $chapter->id]) }}" method="POST">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="px-8 py-3 bg-rose-600 text-white font-bold rounded-xl text-sm hover:bg-rose-700 transition-all shadow-lg shadow-rose-200 dark:shadow-none" onclick="return confirm('Are you sure you want to delete this chapter?')">Delete Chapter</button>
                </form>
            </div>
        </div>
    </div>
