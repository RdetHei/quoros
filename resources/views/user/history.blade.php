...
<div class="max-w-4xl mx-auto mb-12">
    ...
    <div class="space-y-4">
        @forelse($histories as $history)
            <div class="group flex flex-col sm:flex-row gap-6 p-6 bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 hover:border-indigo-500 hover:shadow-xl transition-all relative overflow-hidden">
                ...
                <div class="flex flex-col justify-center flex-grow">
                    ...
                    <div class="flex flex-wrap items-center gap-4 mt-auto">
                        <div class="px-4 py-2 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl border border-indigo-100 dark:border-indigo-800">
                            <p class="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-0.5 text-center">Terakhir Dibaca</p>
                            <p class="text-xs font-bold text-indigo-600 dark:text-indigo-400 text-center">{{ $history->chapter->title }}</p>
                        </div>
                        <a href="{{ route('chapters.show', [$history->novel->slug, $history->chapter->slug]) }}" class="px-6 py-2 bg-indigo-600 text-white font-bold rounded-xl text-sm hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200 dark:shadow-none">Lanjutkan</a>
        ...
        @empty
            <div class="py-20 text-center bg-white dark:bg-slate-900 rounded-3xl border border-dashed border-slate-200 dark:border-slate-800">
                ...
                <a href="{{ route('home') }}" class="inline-block mt-4 text-indigo-600 font-bold hover:underline">Mulai Membaca Sekarang</a>
            </div>
        @endforelse
    </div>
