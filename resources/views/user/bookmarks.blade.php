    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
        @forelse($bookmarks as $bookmark)
            <div class="group relative">
                <a href="{{ route('novels.show', $bookmark->novel->slug) }}" class="block">
                    <div class="relative aspect-[3/4] mb-3 rounded-2xl overflow-hidden shadow-md group-hover:shadow-xl group-hover:shadow-rose-500/20 transition-all duration-300 transform group-hover:-translate-y-2 group-hover:scale-[1.03]">
                ...
                <form action="{{ route('bookmarks.toggle', $bookmark->novel->id) }}" method="POST" class="absolute top-2 right-2">
                    @csrf
                    <button type="submit" class="w-8 h-8 flex items-center justify-center bg-white/90 backdrop-blur-sm text-rose-500 rounded-full shadow-lg hover:scale-110 transition-transform">
        ...
        @empty
            <div class="col-span-full py-20 text-center">
                ...
                <a href="{{ route('home') }}" class="inline-block mt-6 px-8 py-3 bg-indigo-600 text-white font-bold rounded-2xl text-sm shadow-xl shadow-indigo-200">Cari Novel</a>
            </div>
        @endforelse
    </div>
