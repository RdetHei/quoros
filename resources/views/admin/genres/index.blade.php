...
<div class="max-w-4xl mx-auto my-12">
    <div class="flex items-center justify-between mb-8">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Manage Genres</h1>
            <p class="text-slate-500 dark:text-slate-400">Add or edit genres for novel classification.</p>
        </div>
        ...
        <div class="overflow-x-auto">
            ...
            <thead>
                <tr class="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                    <th class="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest">Genre Name</th>
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                @forelse($genres as $genre)
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                        <td class="px-6 py-4">
                            <span class="font-bold text-slate-700 dark:text-slate-300">{{ $genre->name }}</span>
                            <span class="block text-[10px] text-slate-400 font-medium uppercase tracking-tighter">{{ $genre->slug }}</span>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <span class="px-3 py-1 bg-slate-100 dark:bg-slate-800 rounded-full text-xs font-bold text-slate-500">{{ $genre->novels_count }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form action="{{ route('admin.genres.destroy', $genre->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this genre?')">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="3" class="px-6 py-12 text-center">
                            <p class="text-slate-500 italic text-sm">No genres added yet.</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
