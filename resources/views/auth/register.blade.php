        ...
        <form action="{{ route('register') }}" method="POST" class="text-left space-y-5 md:space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" 
                    class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl px-5 py-3.5 md:py-4 text-xs md:text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/40 focus:bg-white dark:focus:bg-slate-900 transition-all @error('name') border-red-500 @enderror" 
            ...
            <div x-data="{ show: false }">
                ...
                <div class="relative">
                    <input :type="show ? 'text' : 'password'" name="password" id="password" 
                        class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl px-5 py-3.5 md:py-4 pr-12 text-xs md:text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/40 focus:bg-white dark:focus:bg-slate-900 transition-all @error('password') border-red-500 @enderror" 
            ...
            <button type="submit" class="w-full py-3.5 md:py-4 bg-slate-900 text-white font-bold rounded-2xl hover:bg-slate-800 transition-all shadow-xl shadow-slate-200 dark:shadow-none text-sm md:text-base">Daftar Sekarang</button>

            <p class="text-center text-xs md:text-sm text-slate-500 dark:text-slate-400">
                Sudah punya akun? 
                <a href="{{ route('login') }}" class="font-bold text-emerald-600 hover:text-emerald-500">Masuk Sini</a>
            </p>
        </form>
    </div>
</div>
