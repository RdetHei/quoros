        ...
        @forelse($novels as $novel)
            <a href="{{ route('novels.show', $novel->slug) }}" class="group">
                <div class="relative aspect-[3/4] mb-3 rounded-2xl overflow-hidden shadow-md group-hover:shadow-xl group-hover:shadow-indigo-500/20 transition-all duration-300 transform group-hover:-translate-y-2 group-hover:scale-[1.03]">
                    ...
                    <div class="absolute bottom-2 left-2 right-2 flex justify-between items-end">
                        <div class="flex flex-col gap-1">
                            ...
                            @endphp
                            <span class="px-1.5 py-0.5 text-[7px] font-black {{ $typeBadge }} text-white rounded uppercase tracking-tighter shadow-lg w-max">{{ str_replace('_', ' ', $novel->type) }}</span>
                            <span class="px-1.5 py-0.5 text-[7px] font-black {{ $ratingBadge }} text-white rounded uppercase tracking-tighter shadow-lg w-max">{{ $novel->content_rating }}</span>
                        </div>
                        
                        @php
                            $statusBadge = match($novel->status) {
                        ...
                        @endphp
                        <span class="px-2 py-0.5 text-[8px] font-black {{ $statusBadge }} text-white rounded uppercase tracking-tighter shadow-lg">{{ $novel->status }}</span>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between">
                    <p class="text-[10px] text-slate-500 dark:text-slate-400 font-medium">{{ $novel->author->name }}</p>
                    <div class="flex items-center gap-2">
                        ...
                        <span class="flex items-center gap-0.5 text-[10px] font-bold text-slate-400">
